...
//    for(int i=0;i<x*y;i++)
//        lockarr[i]=false;
    cout<<"time startes now"<<endl;
    while(flag==0)
    {
        t=counter;
        //#pragma omp parallel for schedule(dynamic,sz/4)
        for(int i=0;i<sz;i++)
        {
...
//            }
     ...
     for(int i=0;i<x*y;i++)
    {
        int xcoord=i/y;
        int ycoord=i%y;
        int rcx=root(Array,i)/y;
        int rcy=root(Array,i)%y;
